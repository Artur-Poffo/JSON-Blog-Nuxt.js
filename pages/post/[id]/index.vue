<template>
  <article v-if="post" class="w-full max-w-screen-lg mx-auto pt-32 px-4 pb-10 flex flex-col gap-12">
    <header class="flex flex-col gap-2">
      <img class="w-full rounded-md" :src="post.image_url" alt="Imagem do post">

      <div class="mt-4">
        <h1 class="text-4xl font-mono font-bold text-sky-400">{{ post.title }}</h1>
...
async function getPostDetails() {
  const { data: { value }, error } = await useMyFetch<IPost>(`/posts/${route.params.id}`);
